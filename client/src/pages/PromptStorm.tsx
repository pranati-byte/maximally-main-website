...
import { Zap, Users, Clock, Globe, Code, Sparkles, MessageSquare, Trophy, Target, Star, ExternalLink, FileText, Video } from 'lucide-react';
...
export default function PromptStorm() {
  ...
  const fastFacts = [
    ...
    { icon: <Code className="h-5 w-5" />, label: "Tools", value: "Any AI tool allowed" },
  ...
  const howItWorksSteps = [
    { step: 1, title: "Register and join the kickoff space", icon: <Users className="h-6 w-6" /> },
    { step: 2, title: "Pick a direction. Visuals, code, stories, audio, games, anything AI-generated.", icon: <Target className="h-6 w-6" /> },
    { step: 3, title: "Make prompts the core. Iterate fast, keep logs.", icon: <MessageSquare className="h-6 w-6" /> },
    { step: 4, title: "Submit on time with links and your best prompt examples.", icon: <FileText className="h-6 w-6" /> }
  ];
  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <div className="relative z-10">
        ...
        <section className="min-h-screen flex flex-col items-center justify-center relative px-4 pt-20">
          ...
          <div className="absolute bottom-10 left-1/2 transform -translate-x-1/2">
            <div className="flex space-x-4 opacity-60">
              <MessageSquare className="h-6 w-6 text-blue-400 animate-bounce" style={{ animationDelay: '0s' }} />
              <Code className="h-6 w-6 text-yellow-400 animate-bounce" style={{ animationDelay: '0.2s' }} />
              <Sparkles className="h-6 w-6 text-red-500 animate-bounce" style={{ animationDelay: '0.4s' }} />
            </div>
          </div>
        </section>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              {fastFacts.map((fact, index) => (
                <div key={index} className="text-center group">
                  ...
                  <div className="font-jetbrains text-sm text-blue-400 font-bold">{fact.value}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="container mx-auto max-w-4xl text-center">
            ...
            <div className="bg-gradient-to-br from-blue-900/30 to-black p-8 rounded-lg border-2 border-blue-400">
              <p className="font-jetbrains text-lg md:text-xl text-white leading-relaxed">
                A 24 hour AI prompt-engineering hackathon where speed, creativity, and a bit of chaos collide. 
                Bend tools to your will. Make prompts that hit. No code needed, ideas welcome.
              </p>
            </div>
          </div>
        </section>
          ...
          <div className="container mx-auto max-w-4xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {whoJoins.map((person, index) => (
                <div key={index} className="bg-gradient-to-br from-yellow-600/20 to-black p-4 rounded-lg border border-yellow-400 hover:scale-105 transition-all duration-300">
                  <div className="flex items-center gap-3">
                    <Sparkles className="h-5 w-5 text-yellow-400" />
                    <span className="font-jetbrains text-white font-medium">{person}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="container mx-auto max-w-5xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {howItWorksSteps.map((step, index) => (
                <Card key={index} className="bg-gradient-to-br from-blue-900/30 to-black border-2 border-blue-400 hover:scale-105 transition-all duration-300">
                  <CardContent className="p-6">
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <p className="font-jetbrains text-white leading-relaxed">{step.title}</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
          ...
          <div className="container mx-auto max-w-4xl">
            ...
            <div className="space-y-6">
              {timeline.map((item, index) => (
                <div key={index} className="flex items-center gap-6 bg-gradient-to-r from-yellow-600/20 to-black p-6 rounded-lg border border-yellow-400">
                  <div className="text-center flex-shrink-0">
                    <div className="font-press-start text-lg text-yellow-400">{item.date}</div>
                    <div className="font-jetbrains text-sm text-blue-400">{item.time}</div>
                  </div>
                  <div className="w-px h-12 bg-gradient-to-b from-yellow-400 to-blue-400"></div>
                  <div className="font-jetbrains text-white text-lg">{item.event}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="container mx-auto max-w-4xl">
              ...
              <div className="grid gap-4">
                {judgingCriteria.map((item, index) => (
                  <div key={index} className="flex justify-between items-center">
                    <span className="font-jetbrains text-white">{item.criteria}</span>
                    <span className="font-press-start text-purple-400">{item.weight}</span>
                  </div>
                ))}
              </div>
            </div>
            <div className="text-center">
              <p className="font-jetbrains text-lg text-gray-300">
                <span className="text-yellow-400 font-bold">Judges:</span> AI builders, designers, creators
              </p>
            </div>
          </div>
        </section>
          ...
          <div className="container mx-auto max-w-5xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {prizes.map((prize, index) => (
                <Card key={index} className="bg-gradient-to-br from-yellow-600/20 to-black border-2 border-yellow-400 hover:scale-105 transition-all duration-300">
                  <CardContent className="p-6 text-center">
                    <div className="mb-4 flex justify-center">{prize.icon}</div>
                    <h3 className="font-press-start text-lg text-white mb-3 leading-tight">{prize.title}</h3>
                    <p className="font-jetbrains text-yellow-400 font-bold">{prize.prize}</p>
                  </CardContent>
                </Card>
              ))}
            </div>
        ...
        <section className="py-20 px-4">
          <div className="container mx-auto max-w-4xl text-center">
            <div className="bg-gradient-to-r from-blue-600/20 via-black to-red-600/20 p-12 rounded-lg border-2 border-yellow-400">
              <h2 className="font-press-start text-2xl md:text-3xl mb-6 text-white">
                READY TO STORM THE PROMPTS?
              </h2>
              <Button 
                onClick={() => window.open('#register', '_self')}
                className="pixel-button bg-gradient-to-r from-yellow-500 to-yellow-600 text-black font-press-start py-6 px-12 text-xl hover:scale-105 transition-all duration-300 border-2 border-blue-400 shadow-xl"
              >
